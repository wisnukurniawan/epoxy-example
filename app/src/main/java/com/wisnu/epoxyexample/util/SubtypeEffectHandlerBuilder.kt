...
class SubtypeEffectHandlerBuilder<F, E> {
    ...
    inline fun <reified G : F> addTransformer(
        ...
        return apply {
            builder.addTransformer(G::class.java, transformer)
        }
    }
    ...
    inline fun <reified G : F> addFunction(
        ...
        return apply {
            scheduler?.run {
                builder.addFunction(G::class.java, { block(it) }, this)
    ...
    inline fun <reified G : F> addAction(
        ...
        return apply {
            scheduler?.run {
                builder.addAction(G::class.java, { block() }, this)
    ...
    inline fun <reified G : F> addConsumer(
        ...
        return apply {
            scheduler?.run {
                builder.addConsumer(G::class.java, { block(it) }, this)
            } ?: builder.addConsumer(G::class.java) { block(it) }
        }
    }

    fun withFatalErrorHandler(
        block: (ObservableTransformer<out F, E>) -> Consumer<Throwable>
    ): SubtypeEffectHandlerBuilder<F, E> {
        return apply {
            builder.withFatalErrorHandler { block(it) }
        }
    }

    fun build(): ObservableTransformer<F, E> {
